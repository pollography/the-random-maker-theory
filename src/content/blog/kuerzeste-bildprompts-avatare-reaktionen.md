...
```prompt
Erstelle aus diesem Sticker eine PNG-Datei mit transparentem Hintergrund. Erhalte Gesicht, orange Brille, Bart, türkisen Hoodie und die gleichmäßige weiße Kontur. Kein Schatten außerhalb der Kontur, kein Text und kein Logo.
```
...
```prompt
Wähle aus dem 3×3-Stickerpack Reihe 2, Spalte 2, gezählt von oben links. Gemeint ist die positive Herzgeste. Erstelle nur diesen Sticker als einzelnes quadratisches Bild mit transparentem Hintergrund, sauberer weißer Kontur und 12 Prozent Rand. Keine weiteren Sticker, kein Text und kein Logo.
```
...
```prompt
Nutze das Emoji-Pack als Stilreferenz. Erstelle sechs einzelne Porträt-Emojis derselben Person: lachen, staunen, nachdenken, feiern, müde und genervt. Einheitlicher Illustrationsstil, transparente Hintergründe, keine zusätzlichen Symbole, kein Text und keine Logos.
```
...
```prompt
Wähle aus dem 3×4-Reaktionsraster Reihe 1, Spalte 3, gezählt von oben links. Gemeint ist die überraschte Reaktion mit offenem Mund. Erstelle ein einzelnes 1:1-Porträt auf transparentem Hintergrund. Erhalte Identität, Brille, Bart und Hoodie. Kein Rahmen, kein Text und kein Logo.
```
...
```prompt
Wähle aus dem 2×2-Avatar-Raster Reihe 1, Spalte 1. Erstelle daraus ein einzelnes quadratisches Profilporträt mit ruhigem hellgrauem Hintergrund. Augen auf gleicher Höhe, Gesicht mittig, 20 Prozent Freiraum über dem Kopf. Identität erhalten, kein Text und kein Logo.
```
...
```prompt
Erstelle aus der Miniatur eine realistische 16:9-Produktaufnahme. Die Figur steht auf einem kleinen schwarzen Sockel auf einem Maker-Schreibtisch. Person und Kleidung bleiben erkennbar. Makroaufnahme, geringe Schärfentiefe, freie Fläche rechts, kein Text und keine Marke.
```
...
```prompt
Erstelle eine glaubwürdige 16:9-Filmszene auf Straßenniveau. Die riesige Person steht zwischen Hochhäusern, wird aus leichter Froschperspektive fotografiert und schaut freundlich nach unten. Identität und türkiser Hoodie erhalten. Realistische Schatten, keine Schrift, keine Logos und keine zerstörte Stadt.
```
