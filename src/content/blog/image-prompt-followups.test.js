...
test('all 86 short-prompt entries explain, universalize and prove their second image step', async () => {
	const sections = [];
	...
	for (const { body, command, file, number } of sections) {
		...
		assert.match(body, /\*\*Was der zweite Prompt macht:\*\*\s+\S/, `${label} needs a concrete purpose`);
		assert.match(body, /\*\*Dafür hochladen:\*\*\s+\S/, `${label} needs explicit inputs`);
		assert.match(body, /\*\*Kopierbare Vorlage:\*\*/, `${label} needs the reusable template label`);

		const blocks = promptBlocks(body);
		assert.ok(blocks.length >= 2, `${label} needs short and detailed prompt blocks`);
		for (const block of blocks.filter((prompt) => !prompt.startsWith('/'))) {
			assert.doesNotMatch(block, hardcodedTestSubject, `${label} hardcodes the test portrait`);
		}

		const reusablePrompt = blocks.at(-1);
		if (reusablePrompt.includes('[[')) {
			assert.match(
				body,
